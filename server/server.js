// ═══════════════════════════════════════════════════════════════
// CRYPTO ROULETTE — Authoritative Server v2
// ═══════════════════════════════════════════════════════════════
...
let gameState = {
  phase: 'waiting', roundId: 39253, players: [], totalPool: 0,
  ...
  function squarify(items, x, y, w, h) {
    ...
    if (items.length === 1) {
      ...
      sectors.push({
        ...
        initials: p.initials, color: p.color, bet: p.bet,
        percent: (p.bet / totalPool * 100).toFixed(1),
        x, y, w, h, cx: x + w / 2, cy: y + h / 2,
      });
      return;
    }
    const totalVal = items.reduce((s, p) => s + p.bet, 0);
    const vertical = w >= h;
    let bestRow = [items[0]], bestRatio = Infinity;
    for (let i = 0; i < items.length; i++) {
      const row = items.slice(0, i + 1);
      const rowVal = row.reduce((s, p) => s + p.bet, 0);
      const rowFrac = rowVal / totalVal;
      const strip = vertical ? w * rowFrac : h * rowFrac;
      let worst = 0;
      for (const p of row) {
        const pf = p.bet / rowVal;
        const cw = vertical ? strip : w * pf;
        const ch = vertical ? h * pf : strip;
        worst = Math.max(worst, Math.max(cw / Math.max(ch, 0.1), ch / Math.max(cw, 0.1)));
      }
      if (worst <= bestRatio || i === 0) { bestRatio = worst; bestRow = row; }
      else break;
    }
    const rowVal = bestRow.reduce((s, p) => s + p.bet, 0);
    const strip = vertical ? w * (rowVal / totalVal) : h * (rowVal / totalVal);
    let offset = 0;
    for (const p of bestRow) {
      const pf = p.bet / rowVal;
      const cell = vertical ? h * pf : w * pf;
      const sx = vertical ? x : x + offset, sy = vertical ? y + offset : y;
      const sw = vertical ? strip : cell, sh = vertical ? cell : strip;
      sectors.push({
        playerId: p.id, name: p.name, avatar: p.avatar,
        initials: p.initials, color: p.color, bet: p.bet,
        percent: (p.bet / totalPool * 100).toFixed(1),
        x: sx, y: sy, w: sw, h: sh, cx: sx + sw / 2, cy: sy + sh / 2,
      });
      offset += cell;
    }
    const rest = items.slice(bestRow.length);
    if (rest.length > 0) {
      if (vertical) squarify(rest, x + strip, y, w - strip, h);
      else squarify(rest, x, y + strip, w, h - strip);
    }
  }
...
function initPuck(rng) {
  ...
  const x = m + rng() * (CONFIG.ARENA_SIZE - 2 * m);
  const y = m + rng() * (CONFIG.ARENA_SIZE - 2 * m);
  const angle = rng() * Math.PI * 2;
  const speed = 18 + rng() * 10;
...
function stepPhysics(puck) {
  ...
  if (puck.x - r <= 0) { puck.x = r; puck.vx = Math.abs(puck.vx) * 0.95; }
  if (puck.x + r >= s) { puck.x = s - r; puck.vx = -Math.abs(puck.vx) * 0.95; }
  if (puck.y - r <= 0) { puck.y = r; puck.vy = Math.abs(puck.vy) * 0.95; }
  if (puck.y + r >= s) { puck.y = s - r; puck.vy = -Math.abs(puck.vy) * 0.95; }
...
function findWinnerSector(puck, sectors) {
  ...
  for (const s of sectors) {
    if (puck.x >= s.x && puck.x <= s.x + s.w && puck.y >= s.y && puck.y <= s.y + s.h) return s;
  }
...
function pickBotName() {
  let a = BOT_NAMES.filter(n => !usedBotNames.has(n));
  if (!a.length) { usedBotNames.clear(); a = BOT_NAMES; }
  const n = a[Math.floor(Math.random() * a.length)];
  usedBotNames.add(n); return n;
}

function getMSKHour() { return (new Date().getUTCHours() + 3) % 24; }

function scheduleBots() {
  const h = getMSKHour();
  let cfg;
  if (h >= 2 && h < 7) cfg = { min: 2, max: 5, wc: 0.002, sc: 0.02 };
  else if (h >= 17 && h < 23) cfg = { min: 6, max: 20, wc: 0.015, sc: 0.12 };
  else cfg = { min: 3, max: 10, wc: 0.005, sc: 0.06 };
  roundCounter++;
  const num = cfg.min + Math.floor(Math.random() * (cfg.max - cfg.min + 1));
  const bots = [];
  for (let i = 0; i < num; i++) {
    let cl = 'mass'; const r = Math.random();
    if (r < cfg.wc && roundCounter % (50 + Math.floor(Math.random() * 150)) === 0) cl = 'whale';
    else if (r < cfg.sc) cl = 'shark';
    else if (r < cfg.sc + 0.25) cl = 'mid';
    let d;
    ...
    bots.push({ name: pickBotName(), bet: generateBotBet(cl), color: BOT_COLORS[Math.floor(Math.random() * BOT_COLORS.length)], delay: d });
  }
  return bots;
}
...
function addBotBet() {
  ...
  const n = pickBotName(), cl = Math.random() < 0.7 ? 'mass' : 'mid';
  addPlayer({ id: 'bot_' + Date.now() + '_' + Math.random().toString(36).substr(2,5),
    name: n, initials: getInitials(n), avatar: null, bet: generateBotBet(cl),
...
function addPlayer(player) {
  ...
  if (['live','spinning','result','paused'].includes(gameState.phase)) return false;
...
function endRound() {
  ...
  const ws = findWinnerSector(gameState.puck, gameState.sectors);
  if (ws) {
    ...
    gameState.winner = { name: ws.name, initials: ws.initials, color: ws.color, amount: winAmount, bet, multiplier: mult, profit, sectorId: ws.playerId };
    if (!ws.playerId.startsWith('bot_') && playerBalances.has(ws.playerId)) {
      playerBalances.set(ws.playerId, Math.round((playerBalances.get(ws.playerId) + profit) * 100) / 100);
      io.to(ws.playerId).emit('balance_update', { balance: playerBalances.get(ws.playerId) });
    }
...
function broadcastState() {
  io.emit('game_state', {
    ...
    puck: gameState.puck ? { x: gameState.puck.x, y: gameState.puck.y, vx: gameState.puck.vx, vy: gameState.puck.vy, spinAngle: gameState.puck.spinAngle, impulsAngle: gameState.puck.impulsAngle, launched: gameState.puck.launched, radius: gameState.puck.radius } : null,
...
server.listen(PORT, () => { console.log(`\n🎰 Crypto Roulette v2 on port ${PORT}\n📡 http://localhost:${PORT}\n`); resetRound(); });