// CRYPTO ROULETTE — Server v8 (trajectory-based)
...
const C={MAX_PLAYERS:32,TIMER:15,COMM:0.15,MIN_BET:0.1,MAX_BET:999999.99,ARENA:600,PKR:16,FRIC:0.991,MIN_SP:0.25,SPIN:3500,RES:4500,PAUSE:3000,INIT_BAL:1000};
...
const pBal=new Map();
let G={ph:'waiting',rid:0,pl:[],pool:0,timer:C.TIMER,ss:null,ps:null,sh:null,winner:null,sectors:[],top:{name:'—',ini:'—',amt:0},last:{name:'—',ini:'—',amt:0},traj:null,trajAngle:0,puckSpawn:null};
let tI=null;
function calcSec(pl,tp,sz){if(!pl.length)return[];const sec=[],so=[...pl].sort((a,b)=>b.bet-a.bet);
  function sq(it,x,y,w,h){if(!it.length)return;if(it.length===1){const p=it[0];sec.push({pid:p.id,name:p.name,ini:p.ini,color:p.color,bet:p.bet,pct:(p.bet/tp*100).toFixed(1),x,y,w,h,cx:x+w/2,cy:y+h/2});return;}
    ...
    for(let i=0;i<it.length;i++){const rw=it.slice(0,i+1),rv=rw.reduce((s,p)=>s+p.bet,0),st=vt?w*(rv/tv):h*(rv/tv);let wst=0;for(const p of rw){const pf=p.bet/rv,cw=vt?st:w*pf,ch=vt?h*pf:st;wst=Math.max(wst,Math.max(cw/Math.max(ch,.1),ch/Math.max(cw,.1)));}if(wst<=brt||i===0){brt=wst;br=rw;}else break;}
    const rv=br.reduce((s,p)=>s+p.bet,0),st=vt?w*(rv/tv):h*(rv/tv);let off=0;
    for(const p of br){const pf=p.bet/rv,cl=vt?h*pf:w*pf,sx=vt?x:x+off,sy=vt?y+off:y,sw=vt?st:cl,sh=vt?cl:st;sec.push({pid:p.id,name:p.name,ini:p.ini,color:p.color,bet:p.bet,pct:(p.bet/tp*100).toFixed(1),x:sx,y:sy,w:sw,h:sh,cx:sx+sw/2,cy:sy+sh/2});off+=cl;}
...
function startSpin(){G.ph='spinning';const rng=sR(G.ss+G.ps),t=calcTraj(rng);G.traj=t.pts;G.trajAngle=t.angle;G.puckSpawn={x:t.sx,y:t.sy};bcast();
  setTimeout(()=>{G.ph='live';io.emit('traj',{pts:G.traj,fps:30});bcast();const dur=G.traj.length/30*1000;setTimeout(()=>endR(),dur+300);},C.SPIN);}
function endR(){G.ph='result';const last=G.traj[G.traj.length-1],ws=findW({x:last[0],y:last[1]});
  if(ws){const p=G.pl.find(pl=>pl.id===ws.pid),bet=p?p.bet:0,np=G.pool-bet,comm=Math.round(np*C.COMM*100)/100,wa=Math.round((G.pool-comm)*100)/100,prof=Math.round((wa-bet)*100)/100,mult=bet>0?Math.round((wa/bet)*100)/100:1,pct=bet>0?(bet/G.pool*100).toFixed(1):'0';
    ...
    if(!ws.pid.startsWith('bot_')&&pBal.has(ws.pid)){const nb=Math.round((pBal.get(ws.pid)+wa)*100)/100;pBal.set(ws.pid,nb);io.to(ws.pid).emit('bal',{b:nb});}
...
io.on('connection',sk=>{console.log(`[+] ${sk.id}`);
  sk.on('restore_bal',d=>{if(typeof d.b==='number'&&d.b>=0)pBal.set(sk.id,Math.round(d.b*100)/100);if(!pBal.has(sk.id))pBal.set(sk.id,C.INIT_BAL);sk.emit('bal',{b:pBal.get(sk.id)});});
  if(!pBal.has(sk.id))pBal.set(sk.id,C.INIT_BAL);sk.emit('bal',{b:pBal.get(sk.id)});bcast();
  ...
  sk.on('bet',d=>{const{name,bet}=d;if(!name||typeof name!=='string')return sk.emit('err',{m:'Неверное имя'});
    ...
    const bal=pBal.get(sk.id)||0;if(ba>bal)return sk.emit('err',{m:`Мало: ${bal} TON`});
    pBal.set(sk.id,Math.round((bal-ba)*100)/100);sk.emit('bal',{b:pBal.get(sk.id)});
    ...
    if(ex){ex.bet=Math.round((ex.bet+ba)*100)/100;ex.name=name.substring(0,20);ex.ini=gI(name);recalc();bcast();sk.emit('bet_ok',{bet:ex.bet});}
      ...
      if(ok)sk.emit('bet_ok',{bet:ba});else{pBal.set(sk.id,Math.round((pBal.get(sk.id)+ba)*100)/100);sk.emit('bal',{b:pBal.get(sk.id)});}}});
